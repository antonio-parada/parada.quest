...
function Bank({ position }: any) {
  return (
    <RigidBody type="fixed" position={position}>
      ...
      <Text position={[0, 7, 0]} fontSize={0.8} color="#ffd700">GASLIGHT_BANK</Text>
    </RigidBody>
  );
}
...
function KarateMan({ onAttack, playerPosRef, kickRange }: any) {
  ...
  useFrame((state) => {
    ...
    if (forward) direction.z -= 1; if (backward) direction.z += 1;
    if (left) direction.x -= 1; if (right) direction.x += 1;
    ...
    if (jump && Math.abs(linvel.y) < 0.1 && pos.y < 1.5) {
       bodyRef.current.setLinvel({ x: linvel.x, y: 13, z: linvel.z }, true);
    }
...
function EmotionalVessel({ data, creepRef, playerPosRef }: any) {
  ...
  const isHealed = false; // Simplified for now

  useFrame(() => {
     if (!creepRef.current || !playerPosRef.current || isHealed) return;
  ...
  return (
    <RigidBody ref={creepRef} position={data.position} colliders="cuboid" lockRotations mass={isHealed ? 100 : 4}>
      <group ref={modelRef}>
        {!isHealed ? (
          <>
          ...
          <group>
            <mesh position={[0, 0, 0]}><cylinderGeometry args={[0.1, 0.1, 1]} /><meshStandardMaterial color="#00ff00" /></mesh>
            <mesh position={[0, 0.6, 0]}><sphereGeometry args={[0.3, 8, 8]} /><meshStandardMaterial color="#ff00ff" /></mesh>
            <Billboard position={[0, 1.5, 0]}>
               <Text fontSize={0.25} color="#00ff00">SIGNAL_INTEGRATED</Text>
            </Billboard>
          </group>
        )}
      </group>
...
function CapacityShard({ position, onCollect }: any) {
  return (
    <Float speed={5} rotationIntensity={2} floatIntensity={2}>
      <RigidBody type="fixed" position={position} sensor onIntersectionEnter={onCollect}>
        ...
        <Text position={[0, 0.8, 0]} fontSize={0.2} color="#00ff00">CAPACITY</Text>
      </RigidBody>
    </Float>
  );
}
...
function Scene() {
  ...
  const [shards, setShards] = useState(() => Array.from({length: 10}).map((_, i) => ({
    id: i, position: [(Math.random() - 0.5) * 100, 1.5, (Math.random() - 0.5) * 100]
  })));
  
  const vessels = useRef(Array.from({length: 15}).map((_, i) => ({
    id: i,
    position: [(Math.random() - 0.5) * 120, 2, (Math.random() - 0.5) * 120],
  ...
  useFrame((state) => {
    if (playerPosRef.current) {
      ...
      if ((state.camera as THREE.PerspectiveCamera).isPerspectiveCamera) {
         (state.camera as THREE.PerspectiveCamera).fov = THREE.MathUtils.lerp((state.camera as THREE.PerspectiveCamera).fov, 45, 0.05);
  ...
  return (
    <>
      ...
      <Physics gravity={[0, -40, 0]}>
        ...
        {shards.map((s) => (
          <CapacityShard key={s.id} position={s.position} onCollect={() => collectShard(s.id)} />
        ))}
        <Bank position={[30, 0, -30]} />
      ...
      <Html fullscreen zIndexRange={[100, 0]}>
         <div className="ui-overlay">
           ...
           <div className="controls">[WASD] REGULATE // [K] HOLD_SPACE<br/>UPLIFT THE VOID.</div>
         </div>
      </Html>
    </>
  );
